...
export class RequestController {
  ...
  async createRequest(c) {
    try {
      ...
      const data = await this.requestService.createRequest(
        payload,
      ...
      const isValidationError =
        /debe ser|invalida|required|requerida|No autorizado/i.test(
          error.message,
        );
      return c.json({
        success: false,
  ...
  async getMyRequests(c) {
    try {
      const data = await this.requestService.getMyRequests(
        this.getAuthenticatedUserId(c),
      );
  ...
  async updateRequestStatus(c) {
    try {
      ...
      const payload = await c.req.json();
      const data = await this.requestService.updateRequestStatus(
        id,
        payload,
        this.getAuthenticatedUserId(c),
        {
      ...
      const isValidationError = /invalido|permiso|no existe/i.test(
        error.message,
      );
      return c.json({
        success: false,
