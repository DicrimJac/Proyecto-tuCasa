...
let currentPropertyOwnerSearch = "";
let currentPropertyIdSearch = "";
...
function normalizeUser(user, index) {
  return {
    ...
    joined: user.date_register || user.created_at || user.createdAt ||
      user.date_created || user.joined || "",
...
async function loadUsersFromDatabase() {
  ...
  try {
    ...
    if (container) {
      container.innerHTML =
        `<tr><td colspan="6" class="text-center">No se pudieron cargar los usuarios registrados</td></tr>`;
    }
...
function normalizePropertyStatus(status) {
  ...
  if (["rechazada", "rechazado", "rejected"].includes(normalized)) {
    return "rejected";
  }
  if (
    ["inactiva", "inactivo", "no disponible", "vendida", "vendido", "inactive"]
      .includes(normalized)
  ) return "inactive";
  return "active";
}
...
function normalizeProperty(property, index) {
  const characteristic = property.caracteristica || property.characteristic ||
    {};
  const id = property.id_propi || property.id || property.property_id ||
    index + 1;
  const stateText = property.state_desc || property.status_desc ||
  ...
  return {
    ...
    title: property.title || property.titulo || property.name ||
      property.type_desc || `Propiedad ${id}`,
    location: getPropertyLocation(property),
    owner: getPropertyOwner(property),
    price: Number(
      characteristic.price ?? property.price ?? property.precio ?? 0,
    ),
    date: property.date_register || property.created_at || property.createdAt ||
      property.date_created || property.date || "",
...
function showPropertiesLoading() {
  ...
  if (container) {
    container.innerHTML =
      `<tr><td colspan="7" class="text-center">Cargando propiedades...</td></tr>`;
  }
}
...
async function loadPropertiesFromDatabase() {
  ...
  try {
    ...
    if (container) {
      container.innerHTML =
        `<tr><td colspan="7" class="text-center">No se pudieron cargar las propiedades registradas</td></tr>`;
    }
...
function getReviewUserName(review) {
  ...
  return getUserFullName(user) || user.mail || "Usuario evaluado";
}

function getReviewPropertyTitle(review) {
  const propertyId = review.id_propi || review.property_id;
  const property = adminProperties.find((item) =>
    String(item.id) === String(propertyId)
  );
  return property?.title ||
    (propertyId ? `Propiedad ${propertyId}` : "Evaluacion de propiedad");
}

function normalizeTenantReview(review) {
  return {
    raw: review,
    id: `tenant-${review.id_review || review.id || Date.now()}`,
    source: "tenant",
    sourceId: review.id_review || review.id,
    propertyTitle: "Evaluacion de arrendatario",
    userName: getReviewUserName(review),
...
function normalizeLandlordReview(review) {
  return {
    raw: review,
    id: `landlord-${review.id_landlord || review.id || Date.now()}`,
    source: "landlord",
    sourceId: review.id_landlord || review.id,
    propertyTitle: "Evaluacion de arrendador",
    userName: getReviewUserName(review),
    date: review.date || review.date_review || "",
    rating: Number(review.total_point || 0),
    categories: {
      comunicacion: Number(review.comunicacion_rank || 0),
      respeto: Number(review.respect_rank || 0),
      mantencion: Number(review.mainte_rank || 0),
      puntualidad: Number(review.timeless_rank || 0),
      transparencia: Number(review.trasparency_rank || 0),
      disponibilidad: Number(review.availability_rank || 0),
      confianza: Number(review.trust_rank || 0),
      experiencia: Number(review.general_exp_rank || 0),
    },
...
function normalizePropertyReview(review) {
  return {
    raw: review,
    id: `property-${review.id_evalupropi || review.id || Date.now()}`,
    source: "property",
    sourceId: review.id_evalupropi || review.id,
    propertyTitle: getReviewPropertyTitle(review),
    userName: "Propiedad evaluada",
    date: review.date_register || review.date || "",
    rating: Number(review.total_point || 0),
    categories: {
      barrio: Number(review.neight || 0),
      servicios: Number(review.service_near || 0),
      seguridad: Number(review.segurity || 0),
      transporte: Number(review.service_bus || 0),
      vecinos: Number(review.neightbors || 0),
      limpieza: Number(review.clean || 0),
      mantencion: Number(review.maintenance || 0),
      precio: Number(review.quality_price || 0),
      ruido: Number(review.level_noise || 0),
      senal: Number(review.signal || 0),
      iluminacion: Number(review.lighting || 0),
      estacionamiento: Number(review.parking || 0),
    },
...
async function fetchReviewCollection(url, normalizer) {
  ...
  return (Array.isArray(result.data) ? result.data : []).map(normalizer);
}
...
async function loadReviewsFromDatabase() {
  ...
  const results = await Promise.allSettled([
    fetchReviewCollection("/api/tenant-reviews", normalizeTenantReview),
    fetchReviewCollection("/api/landlord-reviews", normalizeLandlordReview),
    fetchReviewCollection("/api/property-reviews", normalizePropertyReview),
  ]);

  adminReviews = results
  ...
  failed.forEach((result) =>
    console.error(
      "Error cargando evaluaciones desde la base de datos:",
      result.reason,
    )
  );
  if (failed.length === results.length) {
    reviewsLoadError = "No se pudieron cargar las evaluaciones";
    showToast(reviewsLoadError, true);
  } else if (failed.length > 0) {
    showToast("Algunas evaluaciones no se pudieron cargar", true);
  }
}
...
function simulateRandomData() {
  propertyStats = propertyStats.map((prop) => ({
    ...
    interested: Math.max(
      0,
      prop.interested + Math.floor(Math.random() * 3) - 1,
    ),
  }));
  localStorage.setItem("propertyStats", JSON.stringify(propertyStats));
  initPropertiesChart();
...
function renderRecentProperties() {
  ...
  const recent = [...adminProperties].sort((a, b) =>
  ...
  container.innerHTML = recent.map((prop) => `
        <tr>
            ...
            <td><span class="status-badge status-${prop.status}">${
    escapeHtml(prop.statusText || getStatusText(prop.status))
  }</span></td>
...
function renderUsers() {
  ...
  if (usersLoadError) {
    container.innerHTML = `<tr><td colspan="6" class="text-center">${
      escapeHtml(usersLoadError)
    }</td></tr>`;
  ...
  let filtered = adminUsers.filter((user) =>
    String(user.name || "").toLowerCase().includes(
      currentUserSearch.toLowerCase(),
    ) ||
    String(user.email || "").toLowerCase().includes(
      currentUserSearch.toLowerCase(),
    )
  );
  ...
  container.innerHTML = paginated.map((user) => `
    ...
    user.status === "active" ? "btn-block" : "btn-approve"
  }" onclick="toggleUserStatus(${user.id})">
                    <i class="bi bi-${
    user.status === "active" ? "ban" : "check-circle"
  }"></i>
                </button>
...
function renderProperties() {
  ...
  if (propertiesLoadError) {
    container.innerHTML = `<tr><td colspan="8" class="text-center">${
      escapeHtml(propertiesLoadError)
    }</td></tr>`;
  ...
  let filtered = adminProperties.filter((prop) => {
    const matchFilter = currentPropertyFilter === "all" ||
      prop.status === currentPropertyFilter;
    const ownerSearch = currentPropertyOwnerSearch.toLowerCase().trim();
    const idSearch = currentPropertyIdSearch.trim();
    const matchOwner = !ownerSearch ||
      String(prop.owner || "").toLowerCase().includes(ownerSearch);
    const matchId = !idSearch || String(prop.id || "").includes(idSearch);
    return matchFilter && matchOwner && matchId;
  });
  ...
  if (paginated.length === 0) {
    container.innerHTML =
      `<tr><td colspan="8" class="text-center">No hay propiedades registradas</td></tr>`;
    document.getElementById("propertiesPagination").innerHTML = "";
    return;
  }
...
function renderReviews() {
  ...
  let filtered = adminReviews.filter((review) => {
    const matchType = currentReviewTypeFilter === "all" ||
      review.source === currentReviewTypeFilter;
    const matchSearch = review.propertyTitle.toLowerCase().includes(
      currentReviewSearch.toLowerCase(),
    ) ||
      review.userName.toLowerCase().includes(
        currentReviewSearch.toLowerCase(),
      ) ||
  ...
  const avgRating = totalReviews > 0
    ? (adminReviews.reduce((acc, r) => acc + r.rating, 0) / totalReviews)
      .toFixed(1)
    : 0;
  ...
  if (paginated.length === 0) {
    container.innerHTML =
      `<div class="text-center py-5">No hay evaluaciones</div>`;
    document.getElementById("reviewsPagination").innerHTML = "";
    return;
  }
  ...
  container.innerHTML = paginated.map((review) => `
    ...
    escapeHtml(review.propertyTitle)
  }
                    </div>
    ...
    Object.entries(review.categories).map(([cat, val]) => `
      ...
      "☆".repeat(5 - val)
            ...
            <div class="review-comment">
                "${escapeHtml(review.comment.substring(0, 150))}${
...
window.viewUser = function (id) {
  ...
  modalBody.innerHTML = `
        <div class="user-details">
            ...
            <p><strong>Estado:</strong> <span class="status-badge status-${user.status}">${
    user.status === "active" ? "Activo" : "Inactivo"
  }</span></p>
        </div>
    `;
...
window.toggleUserStatus = function (id) {
  ...
  if (user) {
    ...
    showToast(
      `Usuario ${
        user.status === "active" ? "activado" : "bloqueado"
      } correctamente`,
    );
  }
};
...
window.deleteUser = async function (id) {
  ...
  if (confirm("¿Eliminar este usuario permanentemente?")) {
    try {
      ...
      if (!response.ok || !result.success) {
        throw new Error(
          result?.message || result?.error || "No se pudo eliminar el usuario",
        );
      }
...
window.viewProperty = function (id) {
  ...
  modalBody.innerHTML = `
    ...
    formatDate(property.date)
            ...
            <p><strong>Estado:</strong> <span class="status-badge status-${property.status}">${
    escapeHtml(property.statusText || getStatusText(property.status))
  }</span></p>
            <p><strong>Metros totales:</strong> ${
    property.totalMeters ?? "Sin informacion"
  }</p>
            <p><strong>Dormitorios:</strong> ${
    property.rooms ?? "Sin informacion"
  }</p>
            <p><strong>Banos:</strong> ${
    property.bathrooms ?? "Sin informacion"
  }</p>
        </div>
    `;
...
window.openReviewModal = function (id) {
  ...
  modalBody.innerHTML = `
    ...
    review.source === "tenant"
      ? "Arrendatario"
      : review.source === "landlord"
      ? "Arrendador"
      : "Propiedad"
  }</p>
    ...
    escapeHtml(review.propertyTitle)
            ...
            <p><strong>Usuario:</strong> ${escapeHtml(review.userName)}</p>
            <p><strong>Fecha:</strong> ${formatDate(review.date)}</p>
            <p><strong>Calificación:</strong> ${
    "★".repeat(Math.round(review.rating))
  }${"☆".repeat(5 - Math.round(review.rating))} (${review.rating})</p>
    ...
    Object.entries(review.categories).map(([cat, val]) => `
            ...
            <p class="review-full-comment">${escapeHtml(review.comment)}</p>
        </div>
    `;
...
async function deleteReviewFromDatabase(id) {
  ...
  const endpointBySource = {
    tenant: "/api/tenant-reviews",
    landlord: "/api/landlord-reviews",
    property: "/api/property-reviews",
  };
  const endpoint = endpointBySource[review.source];
  if (!endpoint || !review.sourceId) {
    showToast("No se pudo identificar la evaluacion a eliminar", true);
    return false;
  }

  try {
    const response = await fetch(
      `${endpoint}/${encodeURIComponent(review.sourceId)}`,
      {
        method: "DELETE",
        credentials: "same-origin",
      },
    ...
    if (!response.ok || !result.success) {
      throw new Error(
        result?.message || result?.error || "No se pudo eliminar la evaluacion",
      );
    }
...
// ===================== FILTROS =====================
document.getElementById("propertyStatusFilter")?.addEventListener(
  "change",
  (e) => {
...
document.getElementById("propertyOwnerFilter")?.addEventListener(
  "input",
  (e) => {
    currentPropertyOwnerSearch = e.target.value;
    currentPropertiesPage = 1;
    renderProperties();
  },
);

document.getElementById("propertyIdFilter")?.addEventListener("input", (e) => {
  currentPropertyIdSearch = e.target.value;
  currentPropertiesPage = 1;
  renderProperties();
});

document.getElementById("reviewTypeFilter")?.addEventListener("change", (e) => {
  currentReviewTypeFilter = e.target.value;
