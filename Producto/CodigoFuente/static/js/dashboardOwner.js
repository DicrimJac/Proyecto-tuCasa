...
function getLoggedOwnerKey() {
  ...
  if (savedData) {
    try {
      ...
      return (user.mail || user.email || user.correo || user.id_usuario ||
        user.id || "owner").toString().trim().toLowerCase();
    } catch (error) {
...
function getLoggedOwnerIdFromStorage() {
  const savedData = localStorage.getItem("userData") ||
    localStorage.getItem("userProfile");
  if (savedData) {
    try {
      ...
      console.error("Error leyendo id del arrendador logueado:", error);
    }
  }
...
async function getLoggedOwnerUserId() {
  const storedId = getLoggedOwnerIdFromStorage();
  ...
  try {
    ...
    console.error("Error obteniendo arrendador actual:", error);
    return "";
  }
}
...
function getOwnerPropertyIds() {
  try {
    const allOwners = JSON.parse(
      localStorage.getItem("ownerPropertyIds") || "{}",
    );
...
async function hydratePropertyImage(property) {
  ...
  try {
    const photosResponse = await fetch(
      `/api/properties/${encodeURIComponent(property.id)}/photos`,
      {
        method: "GET",
        credentials: "include",
      },
...
function normalizeApiRequest(request) {
  ...
  return normalizeRequest({
    ...
    email: getMessageValue(request.message, "Correo") || user.mail ||
      user.email || "",
    phone: getMessageValue(request.message, "Telefono") || user.fono ||
      user.phone || "",
    ...
    status: request.status_nbr || request.status_desc || request.status ||
      "pendiente",
...
async function loadReceivedRequests() {
  ...
  try {
    ...
    console.error(
      "Error cargando solicitudes recibidas desde Supabase:",
      error,
    );
  ...
  receivedRequests = loadedFromApi
    ...
    ? storedRequests.filter((request) =>
      ownerPropertyIds.has(request.propertyId)
    )
    : storedRequests;
...
function buildRecentActivity() {
  ...
  const ratingActivity = propertyRatings.map((review) => ({
    type: "resena",
    title: `Nueva resena: ${review.propertyTitle} (${
      review.rating.toFixed(1)
    })`,
    date: review.date,
  }));

  const landlordRatingActivity = landlordRatings.map((review) => ({
    type: "resena",
    title: `Nueva calificacion de arrendador (${review.rating.toFixed(1)})`,
    date: review.date,
  }));
  ...
  const reviewsByProperty = await Promise.all(
    ownerProperties.map(async (property) => {
      try {
        ...
        console.error(
          `Error cargando calificaciones de propiedad ${property.id}:`,
          error,
        );
        return [];
      }
...
async function loadLandlordRatings() {
  const userId = await getLoggedOwnerUserId();
  if (!userId) {
    landlordRatings = [];
    buildRecentActivity();
    return;
  ...
  try {
    ...
    landlordRatings = (Array.isArray(result.data) ? result.data : [])
      .map((review) => ({
        rating: Number(review.total_point || 0),
        date: review.date,
        comment: review.descr || review.comment || "",
      }))
      .filter((review) => Number.isFinite(review.rating) && review.rating > 0);
  } catch (error) {
    console.error("Error cargando calificaciones de arrendador:", error);
    landlordRatings = [];
  }
...
function updateStats() {
  ...
  const approvedReqs =
    receivedRequests.filter((r) => r.status === "aprobada").length;
  const acceptanceRate = totalReqs > 0
    ? Math.round((approvedReqs / totalReqs) * 100)
    : 0;

  let avgRating = 0;
  if (landlordRatings.length > 0) {
    const sum = landlordRatings.reduce((acc, r) => acc + r.rating, 0);
    avgRating = (sum / landlordRatings.length).toFixed(1);
  }
...
function renderRecentActivity() {
  ...
  if (recent.length === 0) {
    container.innerHTML =
      '<div class="empty-state"><i class="fas fa-calendar-day"></i><p>No hay actividad reciente</p></div>';
    return;
  }
  ...
  container.innerHTML = recent.map((a) => `
    ...
    a.type === "solicitud" ? "envelope" : "star"
  }"></i></div>
...
function viewRequestDetail(requestId) {
  ...
  modalBody.innerHTML = `
        ...
        <div class="detail-row"><span class="detail-label">RUT:</span><span class="detail-value">${
    request.rut || "No informado"
        ...
        <div class="detail-row"><span class="detail-label">Fecha solicitud:</span><span class="detail-value">${
    formatDate(request.date)
  }</span></div>
        <div class="detail-row"><span class="detail-label">Inicio deseado:</span><span class="detail-value">${
    formatDate(request.startDate)
  }</span></div>
        <div class="detail-row"><span class="detail-label">Duracion:</span><span class="detail-value">${
    request.duration || "No informada"
  }</span></div>
        <div class="detail-row"><span class="detail-label">Ocupantes:</span><span class="detail-value">${
    request.occupants || "No informado"
  }</span></div>
        <div class="detail-row"><span class="detail-label">Situacion laboral:</span><span class="detail-value">${
    request.employment || "No informada"
  }</span></div>
        <div class="detail-row"><span class="detail-label">Ingreso mensual:</span><span class="detail-value">$${
...
async function disablePropertyAfterApproval(propertyId) {
  ...
  try {
    ...
    if (!response.ok || !result.success) {
      throw new Error(
        result?.message || result?.error ||
          "No se pudo deshabilitar la propiedad",
      );
    }
    ...
    showToast(
      error.message ||
        "Solicitud aprobada, pero no se pudo deshabilitar la propiedad",
      true,
    );
  }
}
...
async function updateRequestStatus(requestId, nextStatus) {
  ...
  if (apiRequest) {
    ...
    if (failedEmail) {
      ...
      showToast(
        `Solicitud ${
          getStatusText(nextStatus).toLowerCase()
        }, pero fallo un correo de resena`,
        true,
      );
    } else {
  ...
  const requestIndex = storedRequests.findIndex((request) =>
...
function renderProperties() {
  ...
  if (ownerProperties.length === 0) {
    container.innerHTML =
      '<div class="empty-state"><i class="fas fa-home"></i><p>No hay propiedades publicadas</p></div>';
    return;
  }
  ...
  container.innerHTML = ownerProperties.map((p) => `
  ...
  }">${p.active ? "Habilitada" : "Deshabilitada"}</span>
                </div>
    ...
    p.active ? "is-active" : "is-inactive"
  }" type="button" title="${
    p.active ? "Deshabilitar propiedad" : "Habilitar propiedad"
  }" aria-label="${
    p.active ? "Deshabilitar propiedad" : "Habilitar propiedad"
  }" onclick="togglePropertyStatus(${p.id})">
                        <i class="bi bi-${
    p.active ? "toggle-on" : "toggle-off"
  }"></i>
                    </button>
...
function editProperty(id) {
  window.location.href = `publishProperty.html?id=${
...
async function togglePropertyStatus(id) {
  ...
  try {
    ...
    if (!response.ok || !result.success) {
      throw new Error(
        result?.message || result?.error || "No se pudo actualizar el estado",
      );
    }
...
async function deleteProperty(id) {
  ...
  try {
    ...
    if (!response.ok || !result.success) {
      throw new Error(
        result?.message || result?.error || "No se pudo eliminar la propiedad",
      );
    }
