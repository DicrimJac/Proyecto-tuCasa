...
const propertyReviewsSection = document.getElementById(
  "propertyReviewsSection",
);
const propertyReviewsSubtitle = document.getElementById(
  "propertyReviewsSubtitle",
);
const propertyAverageScore = document.getElementById("propertyAverageScore");
...
function generateReviewStars(score) {
...
function getReviewScore(review) {
  const score = Number(
    review?.total_point ?? review?.total_rank ?? review?.rating ?? 0,
  );
...
function getPropertyType(rawProperty) {
  return rawProperty.propertyType || rawProperty.type_desc ||
    rawProperty.raw?.type_desc || rawProperty.type || rawProperty.category ||
    "Sin tipo";
}

function getOperationType(rawProperty) {
  const operation = rawProperty.operationType || rawProperty.operation_desc ||
    rawProperty.operation || rawProperty.type_operation || rawProperty.type ||
    "arriendo";
  return normalizeText(operation).includes("venta") ? "Venta" : "Arriendo";
}
...
function getCachedPropertyImage(propertyId) {
  try {
    const cache = JSON.parse(
      localStorage.getItem("propertyImageCache") || "{}",
    );
...
function isPropertyPublic(rawProperty) {
  ...
  const stateText = normalizeText(
    rawProperty?.state_desc || rawProperty?.status_desc ||
      rawProperty?.estado || rawProperty?.status ||
      rawProperty?.raw?.state_desc,
  );
...
function normalizeProperty(rawProperty) {
  const characteristic = rawProperty.caracteristica ||
    rawProperty.characteristic || rawProperty.raw?.caracteristica || {};
  const priceValue = Number(
    characteristic.price ?? rawProperty.price ?? rawProperty.precio ?? 0,
  );
  const operation = getOperationType(rawProperty);
  const area = characteristic.total_mtr ?? rawProperty.area ??
    rawProperty.propertyArea ?? 0;
  const id = rawProperty.id_propi || rawProperty.id ||
    rawProperty.property_id || rawProperty.raw?.id_propi || "";
  ...
  return {
    ...
    price: priceValue
      ? `$${priceValue.toLocaleString()}${
        operation === "Arriendo" ? " / mes" : ""
      }`
      : "Precio no disponible",
    location: getPropertyLocation(rawProperty),
    bedrooms: characteristic.qty_room ?? rawProperty.rooms ??
      rawProperty.bedrooms ?? 0,
    bathrooms: characteristic.qty_bath ?? rawProperty.bathrooms ??
      rawProperty.banos ?? 0,
    parkingSpaces: characteristic.h_parkin
      ? 1
      : (rawProperty.parkingSpaces ?? rawProperty.parking ?? 0),
    propertyArea: area ? `${area} m2` : "Sin información",
    propertyType: getPropertyType(rawProperty),
    operationType: operation,
    description: rawProperty.describe || rawProperty.description ||
      rawProperty.descripcion || characteristic.description ||
...
function renderPropertyReviews(reviews = []) {
  if (!propertyReviewsSection || !propertyReviewsList) return;
  ...
  if (propertyAverageScore) propertyAverageScore.textContent = averageText;
  if (propertyAverageStars) {
    propertyAverageStars.textContent = generateReviewStars(average);
  }
  if (propertyReviewsCount) {
    propertyReviewsCount.textContent = `${totalReviews} ${
      totalReviews === 1 ? "evaluacion" : "evaluaciones"
    }`;
  }
  if (propertyReviewsSubtitle) {
    propertyReviewsSubtitle.textContent = totalReviews
      ? `Ranking promedio ${averageText} de 5 basado en las evaluaciones recibidas.`
      : "Esta propiedad aun no tiene evaluaciones publicadas.";
  }

  if (!totalReviews) {
    propertyReviewsList.innerHTML = `
            <div class="reviews-empty">
                Aun no hay evaluaciones para esta propiedad.
            </div>
        `;
    return;
  }

  propertyReviewsList.innerHTML = validReviews.slice(0, 4).map((review) => {
    const score = getReviewScore(review);
    return `
            <article class="property-review-card">
                <div class="property-review-header">
                    <div>
                        <strong>Evaluacion de propiedad</strong>
                        <span>${
      formatReviewDate(review.date_register || review.date || review.created_at)
    }</span>
                    </div>
                    <div class="property-review-rating" aria-label="Valoracion ${
      score.toFixed(1)
    } de 5">
                        <span>${generateReviewStars(score)}</span>
                        <strong>${score.toFixed(1)}</strong>
                    </div>
                </div>
                <p>${
      escapeHtml(review.description || review.comment || "Sin comentario")
    }</p>
            </article>
        `;
  }).join("");
...
async function loadPropertyReviews(propertyId) {
  ...
  propertyReviewsList.innerHTML =
    '<div class="reviews-empty">Cargando evaluaciones...</div>';

  try {
    const response = await fetch(
      `/api/property-reviews?id_propi=${encodeURIComponent(propertyId)}`,
      {
        method: "GET",
        credentials: "include",
      },
    ...
    renderPropertyReviews(Array.isArray(result.data) ? result.data : []);
  } catch (error) {
    console.error("Error cargando evaluaciones de propiedad:", error);
    if (propertyReviewsSubtitle) {
      propertyReviewsSubtitle.textContent =
        "No se pudieron cargar las evaluaciones.";
    }
    propertyReviewsList.innerHTML = `
            <div class="reviews-empty">
                No se pudieron cargar las evaluaciones de esta propiedad.
            </div>
        `;
  }
}
...
function renderSectorData(propertyData) {
  ...
  if (metroCercano) {
    metroCercano.textContent = propertyData.metro_cercano ||
      "No hay estación de metro cercana";
  }
  if (distanciaMetro) {
    distanciaMetro.textContent = propertyData.distancia_metro
      ? `(${propertyData.distancia_metro})`
      : "";
  }
  ...
  if (saludCount) {
    saludCount.textContent = servicios.salud ?? servicios.health ?? "0";
  }
  if (educacionCount) {
    educacionCount.textContent = servicios.educacion ?? servicios.education ??
      "0";
  }
  if (areasVerdesCount) {
    areasVerdesCount.textContent = servicios.areasVerdes ??
      servicios.green_areas ?? "0";
  }
  if (gastronomiaCount) {
    gastronomiaCount.textContent = servicios.gastronomia ??
      servicios.restaurants ?? "0";
  }
  if (farmaciasCount) {
    farmaciasCount.textContent = servicios.farmacias ?? servicios.pharmacies ??
      "0";
  }
  if (supermercadosCount) {
    supermercadosCount.textContent = servicios.supermercados ??
      servicios.supermarkets ?? "0";
  }
  if (gimnasiosCount) {
    gimnasiosCount.textContent = servicios.gimnasios ?? servicios.gyms ?? "0";
  }
  if (transporteCount) {
    transporteCount.textContent = servicios.transporte ?? servicios.transport ??
      "0";
  }
}
...
function renderAI(propertyData) {
  ...
  if (analisisIA) {
    const analisisTexto = propertyData.analisis_ia || propertyData.analisis ||
      "No hay análisis disponible para este sector.";
    analisisIA.textContent = analisisTexto;
  }
...
async function loadAIAnalysis(propertyData) {
  ...
  if (analisisIA) {
    analisisIA.innerHTML =
      '<i class="fas fa-spinner fa-spin"></i> Analizando sector...';
  }
  ...
  try {
    ...
    if (result.exito && result.data) {
      ...
      if (analisisIA) {
        analisisIA.textContent =
          "El analisis del sector no esta disponible en este momento.";
      }
    }
  } catch (error) {
...
function renderSectorData(data) {
  ...
  if (distanciaMetro) {
    distanciaMetro.textContent = data.distancia_metro
      ? `(${data.distancia_metro})`
      : "";
  }
  ...
  if (saludCount) {
    saludCount.textContent = indicadores.salud || data.salud || "0";
  }
  if (educacionCount) {
    educacionCount.textContent = indicadores.educacion || data.educacion || "0";
  }
  if (areasVerdesCount) {
    areasVerdesCount.textContent = indicadores.areasVerdes ||
      data.areasVerdes || "0";
  }
  if (gastronomiaCount) {
    gastronomiaCount.textContent = indicadores.gastronomia ||
      data.gastronomia || "0";
  }
  if (farmaciasCount) {
    farmaciasCount.textContent = indicadores.farmacias || data.farmacias || "0";
  }
  if (supermercadosCount) {
    supermercadosCount.textContent = indicadores.supermercados ||
      data.supermercados || "0";
  }
  if (gimnasiosCount) {
    gimnasiosCount.textContent = indicadores.gimnasios || data.gimnasios || "0";
  }
  if (transporteCount) {
    transporteCount.textContent = indicadores.transporte || data.transporte ||
      "0";
  }
}
...
async function loadPropertyDetail() {
  ...
  if (propertyId) {
    try {
      const response = await fetch(
        `/api/properties/${encodeURIComponent(propertyId)}?public=true`,
      );
      const result = await response.json();
