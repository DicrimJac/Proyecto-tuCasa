...
function getPropertyCategory(property) {
  const typeText = normalizeText(
    property.type_desc || property.category || property.categoria,
  );
...
function getPropertyCondition(property) {
  const typeText = normalizeText(
    property.type_desc || property.condition || property.condicion,
  );
  return typeText.includes("nueva") || typeText.includes("nuevo")
    ? "nueva"
    : "usada";
}
...
function normalizeProperty(property, index) {
  ...
  return {
    ...
    type: normalizeText(
        property.operation_desc || property.operation ||
    ...
    rooms: Number(
      characteristic.qty_room ?? property.rooms ?? property.habitaciones ?? 0,
    ),
    bathrooms: Number(
      characteristic.qty_bath ?? property.bathrooms ?? property.banos ?? 0,
    ),
    area: Number(
      characteristic.total_mtr ?? property.area ?? property.superficie ?? 0,
    ),
...
async function loadPropertiesFromDatabase() {
  ...
  if (grid) {
    grid.innerHTML =
      `<div class="empty-state"><i class="fas fa-spinner fa-spin"></i><h3>Cargando propiedades</h3></div>`;
  }
  ...
  if (savedFilters) {
    ...
    filteredProperties = allProperties.filter((prop) => {
      ...
      if (
        ...
        !prop.location.toLowerCase().includes(searchFilters.location)
      ) {
        return false;
      }
      ...
      // Filtro condición
      if (
        searchFilters.condition && prop.condition !== searchFilters.condition
      ) {
        return false;
      }
      return true;
    ...
    return `
            <div class="property-card" onclick="viewPropertyDetail(${prop.id})">
                <div class="property-image">
                    <img src="${
      prop.image || DEFAULT_PROPERTY_IMAGE
    }" alt="${prop.title}" onerror="this.src='${DEFAULT_PROPERTY_IMAGE}'">
                    <span class="property-type">${
      prop.type === "arriendo" ? "Arriendo" : "Venta"
    }</span>
                    <div class="property-favorite" onclick="toggleFavorite(event, ${prop.id})">
                        <i class="${
      prop.favorite ? "fas fa-heart" : "far fa-heart"
    }"></i>
                    </div>
                </div>
      ...
      prop.type === "arriendo" ? "/mes" : ""
                    ...
                    <div class="card-tags" style="margin: 0.5rem 0; display: flex; flex-wrap: wrap; gap: 0.4rem;">
                        <span class="categoria" style="background: #eef2f5; color: #2C5A6E; padding: 0.2rem 0.6rem; border-radius: 2rem; font-size: 0.7rem;"><i class="fas fa-building"></i> ${
      categoryNames[prop.category] || prop.category
    }</span>
                        <span class="condicion ${condicionClass}" style="padding: 0.2rem 0.6rem; border-radius: 2rem; font-size: 0.7rem; ${
      prop.condition === "nueva"
        ? "background: #d4edda; color: #155724;"
        : "background: #f8f9fa; color: #6c7f8b;"
    }"><i class="fas fa-star"></i> ${
      conditionNames[prop.condition] || prop.condition
    }</span>
                    </div>
      ...
      formatDate(prop.date)
                        ...
                        <button class="btn-contact" onclick="viewPropertyDetailFromButton(event, ${prop.id})">
...
function toggleFavorite(event, propertyId) {
  ...
  if (property) {
    ...
    const message = property.favorite
...
function viewPropertyDetailFromButton(event, propertyId) {
  event.stopPropagation();
